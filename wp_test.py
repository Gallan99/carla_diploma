...
def main():
    ...
    try:
        ...
        #print the coordinates x,y,z and draw them on the map with red colour.Using next method to take the next avaible waypoint with a distanxe 1cm
        for i in range(0,len(wp)):
            print(str(i)+")"+str(wp[i].transform.location))
            world.debug.draw_string(wp[i].transform.location, 'X', draw_shadow=False,color=carla.Color(r=255, g=0, b=0), life_time=10000,persistent_lines=True)
            next_point.append(wp[i].next(1))
        #print the next avaible waypoint from the list next_point
        for i in next_point:
            for j in i:
                print(j.transform.location)
                world.debug.draw_string(j.transform.location, 'X', draw_shadow=False,color=carla.Color(r=0, g=255, b=0), life_time=10000,persistent_lines=True)
